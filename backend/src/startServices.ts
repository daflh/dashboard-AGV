...
import {
  AgentCondition, AgentConfiguration, AgentsObj, Position2D
} from './models/agent';
...
export default function startServices(httpServer: Server) {
  ...
  function updateAgentsFromDatabase() {
    // TODO: get agents from the database
  ...
  // initial read agents from the database
  updateAgentsFromDatabase();
    
  agentsCommService.onStatusData((agentId, data: AgentCondition) => {
    if (agents[agentId]) {
      // override stored agent data with the new data
      agents[agentId] = { ...agents[agentId], ...data };
      webSocketService.io.to('authenticatedRoom').emit('agent:updated', agentId, data);
    }
  });
  ...
  webSocketService.onClientConnection(  (socket: Socket) => {
    ...
    // get all agents
    socket.on('agent:getAll', (cb: (agentsData: AgentConfiguration[]) => void) => {
    ...
    // send direction control data for 2 Hz
    setInterval(() => {
    ...
    socket.on('agentCmd:targetPosition', (agentId: number, position: Position2D) => {
